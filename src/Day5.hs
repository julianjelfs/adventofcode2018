...
newtype Unit = Unit { unUnit :: Text } deriving Show
...
solution = do
  ...
  pure (partOne inp, partTwo inp)
 where
  partOne = Text.length . unUnit . reactPolymer
  partTwo = findShortest
...
findShortest :: Text -> Int
findShortest t = minimum $ Text.length . unUnit . solveWithout t <$> ['a' .. 'z']
 where
